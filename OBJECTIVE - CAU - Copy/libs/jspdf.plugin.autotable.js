...
var DocHandler = /** @class */ (function () {
    function DocHandler(jsPDFDocument) {
        ...
        this.userStyles = {
            textColor: 0, // Default to black
            fontSize: 10, // Default font size
            fontStyle: 'normal', // Default font style
            font: 'helvetica', // Default font
            lineWidth: 0, // Default line width
            lineColor: 0, // Default line color (black)
        };

        // Safely try to update defaults from the document if it exists
        if (jsPDFDocument) {
            try {
                 // Use safe checks before calling methods
                 this.userStyles.textColor = typeof jsPDFDocument.getTextColor === 'function'
                     ? jsPDFDocument.getTextColor()
                     : this.userStyles.textColor;
                 if (jsPDFDocument.internal) {
                     this.userStyles.fontSize = typeof jsPDFDocument.internal.getFontSize === 'function'
                         ? jsPDFDocument.internal.getFontSize()
                         : this.userStyles.fontSize;
                     const font = typeof jsPDFDocument.internal.getFont === 'function'
                         ? jsPDFDocument.internal.getFont()
                         : null;
                     if (font) {
                          this.userStyles.fontStyle = font.fontStyle || this.userStyles.fontStyle;
                          this.userStyles.font = font.fontName || this.userStyles.font;
                     }
                 }
                 this.userStyles.lineWidth = typeof jsPDFDocument.getLineWidth === 'function'
                     ? jsPDFDocument.getLineWidth()
                     : this.userStyles.lineWidth;
                 this.userStyles.lineColor = typeof jsPDFDocument.getDrawColor === 'function'
                     ? jsPDFDocument.getDrawColor()
                     : this.userStyles.lineColor;
            } catch (e) {
                 console.error("AutoTable DocHandler: Error reading initial styles from jsPDFDocument:", e);
